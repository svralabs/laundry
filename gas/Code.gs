...
// ─────────────────────────────────────────────
// STEP 1: MATERIALIZED SUMMARY CACHE (sheet _summary_cache)
//
// Tujuan: Agar getDashboardStats TIDAK perlu scan semua baris orders/expenses
//         tiap request. Semua metrik sudah dihitung & disimpan di sheet khusus
//         _summary_cache oleh rebuildSummaryCache() yang jalan tiap N menit via trigger.
//
// ⚠ CATATAN TRIGGER:
//   JANGAN panggil setupRebuildTrigger() lewat kode otomatis.
//   Pasang trigger MANUAL via Apps Script Editor → Triggers (ikon alarm):
...
/**
 * STEP 1 — Bangun ulang semua metrik dashboard ke sheet _summary_cache.
 * Dipanggil oleh time-driven trigger setiap 10 menit.
 * Juga bisa dipanggil manual dari editor untuk force-refresh pertama kali.
 */
function rebuildSummaryCache() {
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();

    // ── 1. Baca orders sekali ──────────────────────────────────────────────
    var ordSheet = getOrCreateSheet('orders');
    ...
    if (lastOrdRow > 1) {
      trackSheetsApiCall();
      ordHeaders = ordSheet.getRange(1, 1, 1, HEADERS_MAP['orders'].length).getValues()[0];
      trackSheetsApiCall();
      ordData = ordSheet.getRange(2, 1, lastOrdRow - 1, HEADERS_MAP['orders'].length).getValues();
    }

    // ── 2. Baca expenses sekali ────────────────────────────────────────────
    var expSheet = getOrCreateSheet('expenses');
    ...
    if (lastExpRow > 1) {
      trackSheetsApiCall();
      expHeaders = expSheet.getRange(1, 1, 1, HEADERS_MAP['expenses'].length).getValues()[0];
      trackSheetsApiCall();
      expData = expSheet.getRange(2, 1, lastExpRow - 1, HEADERS_MAP['expenses'].length).getValues();
    }

    // ── 3. Hitung metrik ───────────────────────────────────────────────────
    var today = Utilities.formatDate(new Date(), 'Asia/Jakarta', 'yyyy-MM-dd');
    var now = new Date();
    var currYear = today.slice(0, 4);
    var currMonth = today.slice(0, 7);
    var weekStart = Utilities.formatDate(new Date(now.getTime() - 6 * 86400000), 'Asia/Jakarta', 'yyyy-MM-dd');

    var tIdx = ordHeaders.indexOf('tanggal');
    var sIdx = ordHeaders.indexOf('status');
    var totIdx = ordHeaders.indexOf('total');
    var bIdx = ordHeaders.indexOf('berat');
    ...
    var metrics = {
      ...
      total_berat_month:         0,
      total_berat_year:          0,
      total_customers:           Math.max(0, getOrCreateSheet('customers').getLastRow() - 1)
    };

    for (var i = 0; i < ordData.length; i++) {
      var row = ordData[i];
      var tDate = row[tIdx];
      if (tDate instanceof Date) tDate = Utilities.formatDate(tDate, 'Asia/Jakarta', 'yyyy-MM-dd');
      ...
      metrics.total_orders_all++;
      metrics.revenue_all += total;
      metrics.order_count_all++;

      if (status === 'Masuk')      metrics.status_masuk++;
      else if (status === 'Dicuci')    metrics.status_dicuci++;
      else if (status === 'Disetrika') metrics.status_disetrika++;
      else if (status === 'Selesai')   metrics.status_selesai++;
      else if (status === 'Diambil')   metrics.status_diambil++;

      if (tDate === today) {
        metrics.revenue_today += total;
        metrics.order_count_today++;
        metrics.total_berat_today += berat;
      }
      if (tDate >= weekStart && tDate <= today) {
        metrics.revenue_week += total;
        metrics.order_count_week++;
        metrics.total_berat_week += berat;
      }
      if (tDate.slice(0, 7) === currMonth) {
        metrics.revenue_month += total;
        metrics.order_count_month++;
        metrics.total_berat_month += berat;
      }
      if (tDate.slice(0, 4) === currYear) {
        metrics.revenue_year += total;
        metrics.order_count_year++;
        metrics.total_berat_year += berat;
      }
    }

    // Hitung expense per timeframe
    ...
    if (eTIdx !== -1) {
      for (var e = 0; e < expData.length; e++) {
        var er = expData[e];
        var eDate = er[eTIdx];
        if (eDate instanceof Date) eDate = Utilities.formatDate(eDate, 'Asia/Jakarta', 'yyyy-MM-dd');
        else eDate = (eDate || '').toString().slice(0, 10);
        var amount = parseFloat(er[eJIdx]) || 0;

        metrics.expense_all += amount;
        if (eDate === today)                              metrics.expense_today += amount;
        if (eDate >= weekStart && eDate <= today)         metrics.expense_week  += amount;
        if (eDate.slice(0, 7) === currMonth)              metrics.expense_month += amount;
        if (eDate.slice(0, 4) === currYear)               metrics.expense_year  += amount;
      }
    }

    // ── 4. Tulis ke _summary_cache pakai 1x setValues ─────────────────────
    var cacheSheet = ss.getSheetByName('_summary_cache');
    if (!cacheSheet) {
      cacheSheet = ss.insertSheet('_summary_cache');
      // Sembunyikan sheet dari user awam (bukan delete, hanya hide)
      cacheSheet.hideSheet();
    }
    cacheSheet.clearContents();

    var now_str = new Date().toISOString();
    var rows = [['metric_key', 'metric_value', 'last_updated']];
    for (var key in metrics) {
      rows.push([key, metrics[key], now_str]);
    }

    trackSheetsApiCall();
    cacheSheet.getRange(1, 1, rows.length, 3).setValues(rows);

    // Tandai summary cache sebagai fresh (bukan dirty lagi)
    PropertiesService.getScriptProperties().setProperty('summary_cache_dirty', 'false');
    PropertiesService.getScriptProperties().setProperty('summary_cache_built_at', now_str);

    Logger.log('[rebuildSummaryCache] Done. Rows processed: orders=' + ordData.length + ', expenses=' + expData.length);
    return true;
  } catch(e) {
    Logger.log('[rebuildSummaryCache] ERROR: ' + e.toString());
    return false;
  }
}

/**
 * Baca metrik dari _summary_cache sheet.
 * Return null kalau sheet belum ada / masih dirty.
 */
function readSummaryCache() {
  try {
    var isDirty = PropertiesService.getScriptProperties().getProperty('summary_cache_dirty');
    // Jika belum pernah di-build atau ditandai dirty, kembalikan null → fallback ke on-the-fly
    if (isDirty === null || isDirty === 'true') return null;
    ...
    trackSheetsApiCall();
    var data = cacheSheet.getRange(2, 1, cacheSheet.getLastRow() - 1, 3).getValues();
    var map = {};
    for (var i = 0; i < data.length; i++) {
    ...
    Logger.log('[readSummaryCache] ERROR: ' + e.toString());
    return null;
  }
}
...
function getDashboardStatsData(timeframe) {
  ...
  return getCachedData('dash_stats_v5_' + timeframe, function() {
    ...
    // ── COBA BACA DARI _summary_cache DULU ────────────────────────────────
    // Kalau summary cache tersedia & bukan dirty, bangun response dari situ
    // (tidak perlu scan orders/expenses — O(1) baca ~25 baris sheet kecil).
    // Kalau null → summary belum dibangun / dirty → fallback ke scan on-the-fly.
    var sc = readSummaryCache();
    if (sc !== null) {
      // Pilih metrik yang sesuai timeframe
      var pendapatanPeriod = 0, pengeluaranPeriod = 0, periodOrdersCount = 0, totalBeratPeriod = 0;
      if (timeframe === 'today') {
        pendapatanPeriod   = parseFloat(sc['revenue_today'])      || 0;
        pengeluaranPeriod  = parseFloat(sc['expense_today'])       || 0;
        periodOrdersCount  = parseFloat(sc['order_count_today'])   || 0;
        totalBeratPeriod   = parseFloat(sc['total_berat_today'])   || 0;
      } else if (timeframe === 'week') {
        pendapatanPeriod   = parseFloat(sc['revenue_week'])        || 0;
        pengeluaranPeriod  = parseFloat(sc['expense_week'])        || 0;
        periodOrdersCount  = parseFloat(sc['order_count_week'])    || 0;
        totalBeratPeriod   = parseFloat(sc['total_berat_week'])    || 0;
      } else if (timeframe === 'month') {
        pendapatanPeriod   = parseFloat(sc['revenue_month'])       || 0;
        pengeluaranPeriod  = parseFloat(sc['expense_month'])       || 0;
        periodOrdersCount  = parseFloat(sc['order_count_month'])   || 0;
        totalBeratPeriod   = parseFloat(sc['total_berat_month'])   || 0;
      } else if (timeframe === 'year') {
        pendapatanPeriod   = parseFloat(sc['revenue_year'])        || 0;
        pengeluaranPeriod  = parseFloat(sc['expense_year'])        || 0;
        periodOrdersCount  = parseFloat(sc['order_count_year'])    || 0;
        totalBeratPeriod   = parseFloat(sc['total_berat_year'])    || 0;
      } else { // 'all'
        pendapatanPeriod   = parseFloat(sc['revenue_all'])         || 0;
        pengeluaranPeriod  = parseFloat(sc['expense_all'])         || 0;
        periodOrdersCount  = parseFloat(sc['order_count_all'])     || 0;
      }
      ...
      // recentOrders & activeProgress & chartData tetap diambil via scan singkat (5000 baris terakhir)
      // Ini unavoidable karena butuh data per-baris — tapi jauh lebih hemat karena metrik besar sudah dari cache
      var ordSheet2  = getOrCreateSheet('orders');
      var lastRow2   = ordSheet2.getLastRow();
      var recentOrders2 = [], activeProgress2 = [], chartBuckets2 = [];

      var dayNames2   = ['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab'];
      var monthNames2 = ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun', 'Jul', 'Agus', 'Sep', 'Okt', 'Nov', 'Des'];

      if (timeframe === 'today') {
        var hours2 = ['08:00', '10:00', '12:00', '14:00', '16:00', '18:00', '20:00'];
        for (var h2 = 0; h2 < hours2.length; h2++) chartBuckets2.push({ key: hours2[h2], label: hours2[h2], orders: 0, rev: 0 });
      } else if (timeframe === 'week') {
        for (var d2 = 6; d2 >= 0; d2--) {
          var dt2 = new Date(now.getTime() - d2 * 86400000);
          chartBuckets2.push({ key: Utilities.formatDate(dt2, 'Asia/Jakarta', 'yyyy-MM-dd'), label: dayNames2[dt2.getDay()], orders: 0, rev: 0 });
        }
      } else if (timeframe === 'month') {
        chartBuckets2 = [
          { key: 'w1', label: 'Mgg 1', orders: 0, rev: 0 }, { key: 'w2', label: 'Mgg 2', orders: 0, rev: 0 },
          { key: 'w3', label: 'Mgg 3', orders: 0, rev: 0 }, { key: 'w4', label: 'Mgg 4', orders: 0, rev: 0 }
        ];
      } else if (timeframe === 'year') {
        for (var m2 = 0; m2 < 12; m2++) {
          var mKey2 = currYear + '-' + (m2 < 9 ? '0' + (m2 + 1) : (m2 + 1));
          chartBuckets2.push({ key: mKey2, label: monthNames2[m2], orders: 0, rev: 0 });
        }
      }

      if (lastRow2 > 1) {
        var scanCount2 = Math.min(5000, lastRow2 - 1);
        var startRow2  = lastRow2 - scanCount2 + 1;
        trackSheetsApiCall();
        var headers2 = ordSheet2.getRange(1, 1, 1, HEADERS_MAP['orders'].length).getValues()[0];
        trackSheetsApiCall();
        var data2 = ordSheet2.getRange(startRow2, 1, scanCount2, HEADERS_MAP['orders'].length).getValues();

        var tIdx2   = headers2.indexOf('tanggal');
        var sIdx2   = headers2.indexOf('status');
        var totIdx2 = headers2.indexOf('total');
        var cIdx2   = headers2.indexOf('created_at');
        var bIdx2   = headers2.indexOf('berat');
        var weekStart2 = Utilities.formatDate(new Date(now.getTime() - 6 * 86400000), 'Asia/Jakarta', 'yyyy-MM-dd');

        for (var i2 = data2.length - 1; i2 >= 0; i2--) {
          var row2   = data2[i2];
          var tDate2 = row2[tIdx2];
          if (tDate2 instanceof Date) tDate2 = Utilities.formatDate(tDate2, 'Asia/Jakarta', 'yyyy-MM-dd');
          else tDate2 = (tDate2 || '').toString().slice(0, 10);

          var status2 = (row2[sIdx2] || '').toString();
          var total2  = parseFloat(row2[totIdx2]) || 0;

          var obj2 = null;
          var needsObj2 = (recentOrders2.length < 5) || (activeProgress2.length < 4 && status2 !== 'Diambil');
          if (needsObj2) {
            obj2 = {};
            for (var j2 = 0; j2 < headers2.length; j2++) {
              var v2 = row2[j2];
              if (v2 instanceof Date) v2 = v2.toISOString().slice(0, 10);
              obj2[headers2[j2]] = v2;
            }
            if (recentOrders2.length < 5) recentOrders2.push(obj2);
            if (activeProgress2.length < 4 && status2 !== 'Diambil') activeProgress2.push(obj2);
          }

          // Chartdata fill
          if (chartBuckets2.length > 0) {
            var inTf2 = false;
            if (!timeframe || timeframe === 'all') inTf2 = true;
            else if (timeframe === 'today')  inTf2 = (tDate2 === today);
            else if (timeframe === 'week')   inTf2 = (tDate2 >= weekStart2 && tDate2 <= today);
            else if (timeframe === 'month')  inTf2 = tDate2.startsWith(currMonth);
            else if (timeframe === 'year')   inTf2 = tDate2.startsWith(currYear);

            if (inTf2) {
              if (timeframe === 'today') {
                var crAt2 = obj2 ? (obj2['created_at'] || '') : (row2[cIdx2] || '');
                var hr2 = 8;
                if (crAt2 && crAt2.toString().indexOf('T') !== -1) hr2 = parseInt(crAt2.toString().split('T')[1].split(':')[0], 10) || 8;
                var bkIdx2 = hr2 < 9 ? 0 : hr2 < 11 ? 1 : hr2 < 13 ? 2 : hr2 < 15 ? 3 : hr2 < 17 ? 4 : hr2 < 19 ? 5 : 6;
                if (chartBuckets2[bkIdx2]) { chartBuckets2[bkIdx2].orders++; chartBuckets2[bkIdx2].rev += total2; }
              } else if (timeframe === 'week') {
                for (var b2 = 0; b2 < chartBuckets2.length; b2++) {
                  if (chartBuckets2[b2].key === tDate2) { chartBuckets2[b2].orders++; chartBuckets2[b2].rev += total2; }
                }
              } else if (timeframe === 'month') {
                var dayNum2 = parseInt(tDate2.slice(8, 10), 10) || 1;
                var wIdx2 = Math.min(3, Math.floor((dayNum2 - 1) / 7));
                chartBuckets2[wIdx2].orders++; chartBuckets2[wIdx2].rev += total2;
              } else {
                var mStr2 = tDate2.slice(0, 7);
                for (var b3 = 0; b3 < chartBuckets2.length; b3++) {
                  if (chartBuckets2[b3].key === mStr2) { chartBuckets2[b3].orders++; chartBuckets2[b3].rev += total2; }
                }
              }
            }
          }
      ...
      return {
        totalCustomers:    parseFloat(sc['total_customers'])  || 0,
        todayOrders:       parseFloat(sc['order_count_today']) || 0,
        sedangDicuci:      (parseFloat(sc['status_masuk']) || 0) + (parseFloat(sc['status_dicuci']) || 0) + (parseFloat(sc['status_disetrika']) || 0),
        siapDiambil:       parseFloat(sc['status_selesai'])   || 0,
        pendapatanHariIni: parseFloat(sc['revenue_today'])    || 0,
        pendapatanPeriod:  pendapatanPeriod,
        pengeluaranPeriod: pengeluaranPeriod,
        labaRugiPeriod:    labaRugiPeriod,
        chartData:         chartBuckets2.map(function(b) { return { label: b.label, orders: b.orders, rev: b.rev }; }),
        recentOrders:      recentOrders2,
        activeProgress:    activeProgress2,
        growthPct:         24.5,
        periodOrdersCount: periodOrdersCount,
        averageOrderValue: averageOrderValue,
        totalBeratPeriod:  totalBeratPeriod,
        _source:           'summary_cache'  // debug flag — bisa dihapus setelah stable
      };
    }

    // ── FALLBACK: ON-THE-FLY SCAN (kalau summary cache belum tersedia) ────
    // Ini path lama — tetap dipertahankan, TIDAK dihapus.
    // Akan otomatis dipakai saat pertama kali deploy (sebelum trigger dipasang)
    // atau saat summary_cache_dirty = true.
    Logger.log('[getDashboardStatsData] summary_cache miss — running on-the-fly scan (timeframe=' + timeframe + ')');
...
function invalidateSummaryCache() {
  ...
  // Tandai summary cache sebagai dirty agar getDashboardStatsData
  // tahu bahwa _summary_cache sheet perlu di-rebuild saat dibaca berikutnya
  try {
    PropertiesService.getScriptProperties().setProperty('summary_cache_dirty', 'true');
  } catch(e) {}
}
